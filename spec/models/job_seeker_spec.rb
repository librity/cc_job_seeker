...
describe JobSeeker, type: :model do
  ...
  it 'has relations' do
    ...
    expect(subject).to respond_to(:applied_jobs)
  ...
  context 'validation: email' do
    ...
    it 'must be unique' do
      job_seeker = described_class.new email: subject.email

      expect(job_seeker).to_not be_valid
      expect(job_seeker.errors[:email]).to include(I18n.t('errors.messages.taken'))
    end
  ...
  context 'method: resolve_name' do
    it 'should return social_name if it exists' do
      subject = create :job_seeker, :with_social_name
